...
class BinanceClient:
    ...
    def get_historical_candles(self, contract: Contract, interval: str) -> typing.List[Candle]:
        ...
        # The maximum number of candles is 1000 on Binance Spot
        data['limit'] = 1000
    ...
    def get_bid_ask(self, contract: Contract) -> typing.Dict[str, float]:
        ...
        if self.futures:
            ob_data = self._make_request(
                "GET", "/fapi/v1/ticker/bookTicker", data)
        else:
            ob_data = self._make_request(
                "GET", "/api/v3/ticker/bookTicker", data)
    ...
    def place_order(self, contract: Contract, order_type: str, quantity: float, side: str, price=None, tif=None) -> OrderStatus:
        ...
        data['quantity'] = round(
            int(quantity / contract.lot_size) * contract.lot_size, 8)  # int() to round down
        # Makes sure the order type is in uppercase
        ...
        if order_status is not None:
            ...
            if not self.futures:
                if order_status['status'] == "FILLED":
                    order_status['avgPrice'] = self._get_execution_price(
                        contract, order_status['orderId'])
                else:
        ...
        if "e" in data:
            ...
            if data['e'] == "aggTrade":
                ...
                for key, strat in self.strategies.items():
                    if strat.contract.symbol == symbol:
                        res = strat.parse_trades(float(data['p']), float(
                            data['q']), data['T'])  # Updates candlesticks
    ...
    def subscribe_channel(self, contracts: typing.List[Contract], channel: str):
        ...
        try:
            ...
            logger.info("Binance: subscribing to: %s",
                        ','.join(data['params']))
        except Exception as e:
            logger.error(
                "Websocket error while subscribing to @bookTicker and @aggTrade: %s", e)
    ...
    def get_trade_size(self, contract: Contract, price: float, balance_pct: float):
        ...
        trade_size = round(round(trade_size / contract.lot_size)
                           * contract.lot_size, 8)  # Removes extra decimals

        logger.info("Binance current %s balance = %s, trade size = %s",
                    contract.quote_asset, balance, trade_size)
