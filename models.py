...
class Candle:
    def __init__(self,candle_info,timeframe,exchange):
        if exchange == "binance":
            ...
            #we convert bitmex datetime string to unix timestamp (binance uses unix format as default)
...
class Contract:
    def __init__(self,contract_info,exchange):
        if exchange == "binance":
            ...
            self.quate_asset = contract_info["quoteAsset"]
            ...
            self.price_decimals = tick_to_decimal(contract_info["tickSize"])
            self.quantity_decimals = tick_to_decimal(contract_info["lotSize"])
