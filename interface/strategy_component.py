...
class StrategyEditor(tk.Frame):
    def __init__(self, root: "Root", binance: BinanceClient, bitmex: BitmexClient, *args, **kwargs):
        ...
        self._base_params = [
            ...
            {"code_name": "balance_pct", "widget": tk.Entry,
                "data_type": float, "width": 10, "header": "Balance %"},
            {"code_name": "take_profit", "widget": tk.Entry,
                "data_type": float, "width": 7, "header": "TP %"},
            {"code_name": "stop_loss", "widget": tk.Entry,
                "data_type": float, "width": 7, "header": "SL %"},
        ...
        self._body_frame = ScrollableFrame(
            self._table_frame, bg=BG_COLOR, height=250)
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param['widget'] == tk.OptionMenu:
                ...
                self.body_widgets[code_name][b_index].config(
                    width=base_param['width'], bd=0, indicatoron=0)
        ...
        for param in self.extra_params[strat_selected]:
            ...
            if param['widget'] == tk.Entry:
                ...
                if self.additional_parameters[b_index][code_name] is not None:
                    self._extra_input[code_name].insert(tk.END, str(
                        self.additional_parameters[b_index][code_name]))
            else:
                continue
        ...
        if self.body_widgets['activation'][b_index].cget("text") == "OFF":
            ...
            for param in self._base_params:
                ...
                if code_name != "activation" and "_var" not in code_name:
                    self.body_widgets[code_name][b_index].config(
                        state=tk.DISABLED)  # Locks the widgets of this row

            self.body_widgets['activation'][b_index].config(
                bg="darkgreen", text="ON")
            self.root.logging_frame.add_log(
        ...
        for row in data:
            ...
            for base_param in self._base_params:
                ...
                if base_param['widget'] == tk.OptionMenu and row[code_name] is not None:
                    self.body_widgets[code_name +
                                      "_var"][b_index].set(row[code_name])
                elif base_param['widget'] == tk.Entry and row[code_name] is not None:
                    self.body_widgets[code_name][b_index].insert(
                        tk.END, row[code_name])
