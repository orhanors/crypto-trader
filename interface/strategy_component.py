...
class StrategyEditor(tk.Frame):
    def __init__(self, root, binance: BinanceFuturesClient, bitmex: BitmexClient, *args, **kwargs):
        ...
        for exchange, client in self._exchanges.items():
            for symbol, contract in client.contracts.items():
                ...
                self._all_contracts.append(
                    symbol + "_" + exchange.capitalize())
        ...
        self._headers = ["Contract", "Strategy",
                         "Timeframe", "Balance %", "TP %", "SL %"]
        ...
        for idx, header in enumerate(self._headers):
            header = tk.Label(self._table_frame, text=header,
    ...
    def _add_strategy_row(self):
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param["widget"] == tk.OptionMenu:
                ...
                # set first elements as default
                self.body_widgets[code_name +
                                  "_var"][b_index].set(base_param["values"][0])
                ...
                self.body_widgets[code_name][b_index] = ttk.Combobox(
                    self._table_frame, textvariable=self.body_widgets[code_name+"_var"][b_index], width=15, height=20, justify=tk.CENTER)
                self.body_widgets[code_name][b_index]["values"] = tuple(
                    base_param["values"])
            ...
            self.body_widgets[code_name][b_index].grid(
                row=b_index, column=col, padx=15, pady=15)
    ...
    def _delete_row(self, b_index: int):
        ...
        for element in self._base_params:
            self.body_widgets[element["code_name"]
                              ][b_index].grid_forget()  # remove from ui

            del self.body_widgets[element["code_name"]
                                  ][b_index]  # remove from dict
    ...
    def _show_popup(self, b_index: int):
        ...
        # coordinates of popup window
        self._popup_window.geometry(f"+{x-70}+{y+40}")
        ...
        for param in self._extra_params[strategy_selected]:
            ...
            temp_label = tk.Label(
                self._popup_window, bg=BG_COLOR, fg=FG_COLOR, text=param["name"], font=BOLD_FONT)
            temp_label.grid(row=row_nb, column=0)

            if param["widget"] == tk.Entry:
                self._extra_input[code_name] = tk.Entry(
                    self._popup_window, bg=BG_COLOR_2, justify=tk.CENTER, fg=FG_COLOR, insertbackground=FG_COLOR)
        ...
        for param in self._extra_params[strategy_selected]:
            ...
            if extra_input == "":
                ...
                self._additional_parameters[b_index][code_name] = param["data_type"](
                    extra_input)  # convert str to correct data type
    ...
    def _switch_strategy(self, b_index: int):
        ...
        exchange = self.body_widgets["contract_var"][b_index].get().split("_")[
            1]
        ...
        balance_percentage = float(
            self.body_widgets["balance_percentage"][b_index].get())
        ...
        if self.body_widgets["activation"][b_index].cget("text") == "OFF":
            ...
            if strategy_selected == "Technical":
                new_strategy = TechnicalStrategy(
                    contract, exchange, timeframe, balance_percentage, take_profit, stop_loss, self._additional_parameters[b_index])

            elif strategy_selected == "Breakout":
                new_strategy = BreakoutStrategy(
                    contract, exchange, balance_percentage, take_profit, stop_loss, self._additional_parameters[b_index])
            ...
            if len(new_strategy.candles) == 0:
                self.root.logging_fram.add_log(
            ...
            self.body_widgets["activation"][b_index].config(
                bg=SUCCESS_COLOR, text="ON")

            self.root.logging_frame.add_log(
                f"{strategy_selected} strategy on {symbol}/{timeframe} started")
            ...
            self.body_widgets["activation"][b_index].config(
                bg=DANGER_COLOR, text="OFF")

            self.root.logging_frame.add_log(
                f"{strategy_selected} strategy on {symbol}/{timeframe} stopped")
