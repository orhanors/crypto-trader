...
class TradesWatch(tk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        # Dictionary of dictionaries, contains all the references to the widgets in the table
        self.body_widgets = dict()

        self._headers = ["time", "symbol", "exchange",
                         "strategy", "side", "quantity", "status", "pnl"]
    ...
    def add_trade(self, trade: Trade):
        ...
        # This is the trade row identifier, Unix Timestamp in milliseconds, so should be unique.
        t_index = trade.time

        dt_str = datetime.datetime.fromtimestamp(
            trade.time / 1000).strftime("%b %d %H:%M")
