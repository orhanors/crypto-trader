...
class Autocomplete(tk.Entry):
    ...
    def _changed(self, var_name: str, index: str, mode: str):
        ...
        # Set the content of the tk.Entry widget to uppercase as you type
        ...
        if self._var.get() == "":  # Closes the Listbox when the tk.Entry is empty
            ...
            if not self._lb_open:
                ...
                self._lb.place(x=self.winfo_x() + self.winfo_width(),
                               y=self.winfo_y() + self.winfo_height() + 40)

                self._lb_open = True

            # Finds symbols that start with the characters that you typed in the tk.Entry widget
            symbols_matched = [
                symbol for symbol in self._symbols if symbol.startswith(self._var.get())]
            ...
            if len(symbols_matched) > 0:
                ...
                # Takes only the first 8 elements of the list to match the Listbox
